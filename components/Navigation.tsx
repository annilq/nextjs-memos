...
const Navigation = (props: Props) => {
    ...
    icon: (
      <>
        <div className="relative">
          ...
          <div className="absolute top-0 left-5 w-2 h-2 rounded-full bg-blue-500"></div>
  ...
  return (
    <header
      className={cn(
        "w-full h-full flex flex-col justify-start items-start py-4 md:pt-6 z-30 hide-scrollbar",
        className,
      )}
    >
      ...
      <div className="w-full px-1 py-2 flex flex-col justify-start items-start shrink-0 space-y-2">
        {navLinks.map((navLink) => (
          <ActiveLink
            className={cn(
              "px-2 py-2 rounded-2xl border flex flex-row items-center text-lg text-gray-800 dark:text-gray-300 hover:bg-white hover:border-gray-200 dark:hover:border-zinc-700 dark:hover:bg-zinc-800 border-transparent",
              collapsed ? "" : "w-full px-4",
            )}
            activeClassName="bg-white drop-shadow-sm dark:bg-zinc-800 border-gray-200 dark:border-zinc-700"
            key={navLink.id}
            href={navLink.path}
          >
            {props.collapsed ? (
